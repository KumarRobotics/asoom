...
Eigen::Vector2f Map::getMapImgs(cv::Mat& color, cv::Mat& sem, cv::Mat& sem_viz) const {
  ...
  color = cv::Mat(map_.getSize()(0), map_.getSize()(1), CV_8UC3, cv::Scalar(255,255,255));
  // The default toImage does not properly handle color, so we loop manually
  // This code is modified from that
  const grid_map::Matrix& color_layer = map_["color"];
  ...
  for (grid_map::GridMapIterator it(map_); !it.isPastEnd(); ++it) {
    ...
    const float& sem_viz_value = sem_layer_viz(index(0), index(1));
    const float& color_value = color_layer(index(0), index(1));

    if (std::isfinite(color_value)) {
      const grid_map::Index image_index(it.getUnwrappedIndex());
      color_vec = SemanticColorLut::unpackColor(*reinterpret_cast<const uint32_t*>(&color_value));
      color.at<cv::Vec<uint8_t, 3>>(image_index(0), image_index(1)) = 
        cv::Vec<uint8_t, 3>(color_vec[0], color_vec[1], color_vec[2]);

      if (std::isfinite(sem_viz_value)) {
        sem.at<uint8_t>(image_index(0), image_index(1)) = sem_layer(index(0), index(1));

        color_vec = SemanticColorLut::unpackColor(*reinterpret_cast<const uint32_t*>(&sem_viz_value));
